...
$type = 'Transfer To Other User Account';
...
if (isset($_POST["save"])) 
{
 ...
 // $stmt2 = $db->prepare('SELECT * FROM Accounts WHERE account_number = :accountnumber');
 // $stmt2->execute([
 //   ':accountnumber' => $accountDess
 // ]);
//  $account_dest = $stmt2->fetch(PDO::FETCH_ASSOC);

if($type == 'transfer')  
{
  $r = transferBalance($db, $account, $accountDess, 'transfer', $balance, $memo);
}
else{
  $r = transfer_account($db, $account, $accountDess, 'transfer_account', $balance, $memo);
}
if ($r) {
...
<form method="POST" onsubmit="return validate(this);">
  ...
  <div>
    <label for="transfer">User's Account Number: </label>
    <input type="number" id="accountnumber" name="Account Number" required maxlength="30" />
